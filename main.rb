...
module Reconhecedor include Raabro
  ...
  def negativo(i); seq(nil, i, :menos, :x); end
  
  def soma(i); seq(:soma, i, :y, :mais, :x); end
  def subtracao(i); seq(:subtracao, i, :y, :menos, :x); end
  def multiplicacao(i); seq(:multiplicacao, i, :n, :multi, :y); end
  def divisao(i); seq(:divisao, i, :n, :div, :y); end
  def exponenciacao(i); seq(:exponenciacao, i, :n, :elevado, :y); end

  def exp_parenteses(i); seq(:exp_parenteses, i, :parentesesComeco, :x, :parentesesFinal); end
  #Ordem das operações

  def primaria(i); alt(nil, i, :exponenciacao); end
  def secundaria(i); alt(nil, i, :divisao, :multiplicacao); end
  def terciaria(i); alt(nil, i, :subtracao, :soma); end
  
  #Gramática

  def x(i); alt(nil, i, :terciaria, :y); end
  def y(i); alt(nil, i, :secundaria, :z); end
  def z(i); alt(nil, i, :primaria, :n); end
  def n(i); alt(nil, i, :exp_parenteses, :negativo, :num); end

  def expressao(i); alt(:expressao, i, :soma, :subtracao, :multiplicacao, :divisao, :exponenciacao, :exp_parenteses, :negativo, :num); end
